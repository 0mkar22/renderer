...
function WorkOrder() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await createWorkOrder(workOrderToSubmit);
      ...
      if (error.response) {
        const errorMessage = error.response.data.message || 'Failed to submit work order';
  ...
  const handleDownloadImage = async (index) => {
    ...
    try {
      ...
      link.href = image;
      link.download = `work-order-${entryNumber}-item-${index + 1}.png`;
      ...
      <Paper sx={{ p: 3, mt: 3 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {workItems.map((item, index) => (
            <Paper key={index} id={`work-item-${index}`} sx={{ p: 2, mt: 2, position: 'relative' }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) => handleWorkItemChange(index, 'eventName', e.target.value)}
                    required
                    sx={{ minWidth: 260 }}
                  />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth required sx={{ minWidth: 260 }}>
                    ...
                    <Select
                      value={item.eventVenue}
                      onChange={(e) => handleWorkItemChange(index, 'eventVenue', e.target.value)}
                      label="Event Venue"
                      MenuProps={{ PaperProps: { sx: { minWidth: 260 } } }}
                    >
                      {venues.map((v) => (
                  ...
                  {item.eventVenue === 'Others' && (
                    <TextField
                      fullWidth
                      label="Custom Venue"
                      value={item.customVenue || ''}
                      onChange={(e) => handleWorkItemChange(index, 'customVenue', e.target.value)}
                      required
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) => handleWorkItemChange(index, 'eventTime', e.target.value)}
                    InputLabelProps={{ shrink: true }}
                    required
                    sx={{ minWidth: 260 }}
                  />
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth required sx={{ minWidth: 260 }}>
                    ...
                    <Select
                      value={item.poNpo}
                      onChange={(e) => handleWorkItemChange(index, 'poNpo', e.target.value)}
                      label="PO/NPO"
                      MenuProps={{ PaperProps: { sx: { minWidth: 260 } } }}
                    >
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth required sx={{ minWidth: 260 }}>
                    ...
                    <Select
                      value={item.workMain}
                      onChange={(e) => handleWorkItemChange(index, 'workMain', e.target.value)}
                      label="Work Name"
                      MenuProps={{ PaperProps: { sx: { minWidth: 260 } } }}
                    >
                  ...
                  {item.workMain === 'Others' && (
                    <TextField
                      ...
                      onChange={(e) => handleWorkItemChange(index, 'customWorkMain', e.target.value)}
                      required
                      sx={{ minWidth: 260, mt: 1 }}
                    />
                  )}
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth required sx={{ minWidth: 260 }}>
                    ...
                    <Select
                      value={item.workSub}
                      onChange={(e) => handleWorkItemChange(index, 'workSub', e.target.value)}
                      label="Work Subcategory"
                      disabled={!item.workMain}
                      MenuProps={{ PaperProps: { sx: { minWidth: 260 } } }}
                    >
                      ...
                      {item.workMain === 'Others'
                        ...
                        : Array.isArray(subWorks[item.workMain]) && subWorks[item.workMain].map((sub) => (
                            <MenuItem key={sub} value={sub}>
                              {sub.replaceAll('_', ' ')}
                            </MenuItem>
                          ))}
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) => handleWorkItemChange(index, 'contactPerson', e.target.value)}
                    required
                    sx={{ minWidth: 260 }}
                  />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) => handleWorkItemChange(index, 'contactNumber', e.target.value.replace(/[^0-9]/g, '').slice(0, 10))}
                    required
                    inputProps={{ maxLength: 10, inputMode: 'numeric', pattern: '[0-9]*', style: { textAlign: 'center', fontVariantNumeric: 'tabular-nums' } }}
                    sx={{ minWidth: 260 }}
                  />
                </Grid>
                <Grid item xs={12}>
